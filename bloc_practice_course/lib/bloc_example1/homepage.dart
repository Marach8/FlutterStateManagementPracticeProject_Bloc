...
class BlocExample1 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(centerTitle: true, title: const Text('Flutter Bloc Example1')),
      body: Column(
        children: [
          Row(
            children: [
          ...
          BlocBuilder<PersonBloc, FetchedResults?>(
            ...
            builder: (context, fetchedResults){
              ...
              if (fetchedPersons == null){return const SizedBox();}
              return Expanded(
                child: ListView.builder(
                  itemCount: fetchedPersons.length,
                  itemBuilder: (context, listIndex){
                    final fetchedPerson = fetchedPersons[listIndex]!;
                    return ListTile(title: Text(fetchedPerson.name));
                  }
                ),
              );
            }
